...
User llmr_ Token
  ...
  -> directive-proxy
  -> Vendor 独立内部 listener /api/internal/resolver
  -> Vendor API
```
...
Entry 不验证 `dpr_*`，Vendor 内部 resolver 也不要求它。

## 固定 Remote Token

固定 Token 的 RemoteSpec 只包含 Vendor 内部 resolver URL 和 S2S Bearer，不包含 Route Credential UUID。
HTTP 调用实际由 directive-proxy 发起；S2S Bearer 在 Vendor 内部 listener 上验证。使用下列命令生成：

```bash
export DIRECTIVE_TOKEN_SECRET='same-secret-as-directive-proxy'
export RELAY_ENTRY_S2S_TOKEN='same-secret-as-vendor'
app remote-token --resolver-url 'http://127.0.0.1:23187/api/internal/resolver'
```
...
Vendor 的 `server.resolver.internal-listen` 必须位于 directive-proxy 可达的可信网络，默认同机部署使用
`127.0.0.1:23187`。不得把该 listener 接入公网反向代理；S2S Bearer 是网络隔离之外的第二层校验。
